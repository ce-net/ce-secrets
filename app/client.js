...
export class Vault {
  ...
  // Full login round-trip. rp = { aud?, challenge(): {nonce,aud?,ts?}, submit(proof): Session }.
  async login(rp) {
    const { nonce, aud = rp.aud, ts } = await rp.challenge();
    const proof = await this.signChallenge(aud, nonce, ts);
    return rp.submit(proof);
  }
}

export { C as crypto, V as vault, A as auth };
