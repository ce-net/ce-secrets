...
// ---- app grants (authorization tokens) --------------------------------------
// A grant is a signed authorization an enrolled device issues to an app/audience: "may read
// secrets X, Y until <expiry>". Shape mirrors a ce-cap capability (issuer, audience, abilities,
// expiry, signature) so it can converge with @ce-net/cap later. verifyGrant proves the grant was
// issued by a device enrolled in THIS vault and not revoked/expired — the authorization decision.
// (Crypto note: reading the ciphertext still needs the master key; grants gate the policy/audit
// layer. Per-secret key-wrapping so a grant CANNOT exceed its scope is a planned enhancement.)
const GRANT = (id) => `g.${id}`;

export async function issueGrant(ctx, audience, { read = [], expires } = {}) {
  if (!(await isEnrolled(ctx))) throw new Error('only an enrolled device can issue grants');
  const body = {
    id: grantId(), audience: String(audience),
    abilities: read.map((n) => `read:${n}`),
    issuer: ctx.device.id, issued: now(),
    expires: expires ? expiryToISO(expires) : null,
  };
  const sig = await C.signRecord(ctx.device, body);
  const rec = { ...body, issuerEcdsaPub: ctx.device.ecdsaPub, sig };
  await ctx.store.put(GRANT(body.id), rec);
  return { id: body.id, token: C.enc.b64.enc(C.enc.utf8.enc(JSON.stringify(rec))), grant: rec };
}

export async function listGrants(ctx) {
  return (await ctx.store.list('g.')).map((e) => ({ id: e.value.id, audience: e.value.audience, abilities: e.value.abilities, issuer: e.value.issuer, issued: e.value.issued, expires: e.value.expires }));
}
export async function revokeGrant(ctx, id) { await ctx.store.del(GRANT(id)); }

export async function verifyGrant(ctx, token, audience, action, name) {
  let rec; try { rec = JSON.parse(C.enc.utf8.dec(C.enc.b64.dec(token))); } catch { throw new Error('bad grant token'); }
  const { sig, issuerEcdsaPub, ...body } = rec;
  if (!sig || !issuerEcdsaPub) throw new Error('malformed grant');
  if (!(await C.verifyRecord(issuerEcdsaPub, body, sig))) throw new Error('grant signature invalid');
  const dev = await ctx.store.get(`d.${rec.issuer}`);
  if (!dev) throw new Error('grant issuer is not an enrolled device');
  if (JSON.stringify(dev.ecdsaPub) !== JSON.stringify(issuerEcdsaPub)) throw new Error('grant issuer key mismatch');
  if (!(await ctx.store.get(GRANT(rec.id)))) throw new Error('grant revoked');
  if (rec.expires && Date.parse(rec.expires) < Date.now()) throw new Error('grant expired');
  if (rec.audience !== String(audience)) throw new Error('grant audience mismatch');
  if (!rec.abilities.includes(`${action}:${name}`) && !rec.abilities.includes(`${action}:*`)) throw new Error(`grant does not permit ${action}:${name}`);
  return { ok: true, issuer: rec.issuer, expires: rec.expires };
}

// ---- challenge-response auth (login) ----------------------------------------
// This device signs a fresh server nonce, proving it is an enrolled operator of the vault. The
// relying party then calls verifyAuth (here or via the SDK) to check the signer is in `d.*`.
//
// signChallenge(ctx, aud, nonce) -> AuthProof. `nonce` is the RP's challenge (any string; for the
// stateless variant it is auth.makeNonce(serverSecret, ts)). `ts` defaults to now (ISO-8601).
export async function signChallenge(ctx, aud, nonce, ts = A.nowISO()) {
  const sig = await A.signChallenge(ctx.device, { aud, nonce, ts });
  return {
    deviceId: ctx.device.id,
    ecdsaPub: ctx.device.ecdsaPub,
    aud: String(aud),
    nonce: String(nonce),
    ts,
    sig,
...
// verifyAuth(ctx, aud, nonce, proof) -> { ok, deviceId, label }. Verifies the signature, that the
// signer is an ENROLLED device (its d.<id> ecdsaPub equals the presented one), and that aud/nonce
// match the issued challenge. Replay/expiry of `ts` is the RP's concern (use auth.checkNonce for
// the stateless HMAC nonce); this proves the operator identity binding.
export async function verifyAuth(ctx, aud, nonce, proof) {
  if (!proof || !proof.sig || !proof.ecdsaPub) return { ok: false, reason: 'malformed-proof' };
  if (String(proof.aud) !== String(aud)) return { ok: false, reason: 'aud-mismatch' };
  if (String(proof.nonce) !== String(nonce)) return { ok: false, reason: 'nonce-mismatch' };
  const dev = await ctx.store.get(DEVICE(proof.deviceId));
  if (!dev) return { ok: false, reason: 'not-enrolled' };
  if (JSON.stringify(dev.ecdsaPub) !== JSON.stringify(proof.ecdsaPub)) {
    return { ok: false, reason: 'device-key-mismatch' };
  }
  const ok = await A.verifyAuth(
    proof.ecdsaPub,
    { aud: proof.aud, deviceId: proof.deviceId, nonce: proof.nonce, ts: proof.ts },
    proof.sig,
  );
  if (!ok) return { ok: false, reason: 'bad-signature' };
  return { ok: true, deviceId: proof.deviceId, label: dev.label };
}

function grantId() { return C.enc.hex.enc(C.randomBytes(8)); }
function expiryToISO(s) {
  if (s instanceof Date) return s.toISOString();
  const m = String(s).match(/^(\d+)\s*([smhdw])$/i);
  if (m) { const n = +m[1], u = { s: 1e3, m: 6e4, h: 36e5, d: 864e5, w: 6048e5 }[m[2].toLowerCase()]; return new Date(epochMs() + n * u).toISOString(); }
  const t = Date.parse(s); if (!Number.isNaN(t)) return new Date(t).toISOString();
  throw new Error(`bad expiry: ${s} (use 30d / 24h / 3600s / ISO)`);
}
// Date.now() is fine in Node/browser here (not a workflow script).
